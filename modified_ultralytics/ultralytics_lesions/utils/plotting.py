...
from ultralytics_lesions.utils import IS_COLAB, IS_KAGGLE, LOGGER, TryExcept, ops, plt_settings, threaded
from ultralytics_lesions.utils.checks import check_font, check_version, is_ascii
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        self.lw = (line_width or max(round(sum(im.size if input_is_pil else im.shape) / 2 * 0.003), 2))//4
        if self.pil:  # use PIL
            ...
            self.tf = max(self.lw - 1, 1)//4  # font thickness # Rabih devided it by 4
            self.sf = self.lw / 9  # font scale # 9 was 3 Rabih changed it
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
